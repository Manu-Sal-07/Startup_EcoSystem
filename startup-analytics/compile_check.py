...
\setbeamertemplate{title page}{
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node[anchor=north west, text width=10cm, align=left] at ($(current page.north west)+(2cm,-2cm)$) {
      {\Huge\bfseries\color{SecondaryColor} Startup Ecosystem\\[4mm] Analytics Platform}
    };

    \node[anchor=north west, draw=PrimaryColor, line width=1pt, fill=white, rounded corners=8pt,
          text width=9cm, inner sep=10pt, align=left] at ($(current page.north west)+(2cm,-7cm)$) {
      {\large\bfseries\color{PrimaryColor}Team Members}\\[3mm]
      {\color{TextDark}\sffamily\small
        srijan C Vachadmath \hfill USN 1BM23CD060\\[1mm]
        Manohara Salmani \hfill USN 1BM23CD035\\[1mm]
        Shreejth Shah \hfill USN 1BM23CD057
      }
    };

    \node[anchor=south west, text width=10.5cm, align=left] at ($(current page.south west)+(2cm,1.2cm)$) {
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{scard}{Relational DB Bottleneck}
        \begin{itemize}
          \item Multi-hop graph lookups (e.g. co-investment paths) require heavy recursive SQL JOINs.
      ...
      \begin{scard}{Concurrency \& Oversubscription}
        \begin{itemize}
          \item Concurrent funding calls read stale transaction state, leading to oversubscribed rounds.
    ...
    \begin{column}{0.48\textwidth}
      \begin{scard}{Unverified Data Lifecycles}
        \begin{itemize}
          \item Founders self-report milestone accomplishments without validation.
      ...
      \begin{scard}{Blocking API Execution}
        \begin{itemize}
          \item Computational matchmaking queries block synchronous request loops.
...
\begin{frame}{Project Vision \& Solution Overview}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{scard}{Platform Objective}
        A high-performance startup analytics platform mapping founders, startups, and investor relationships.
        \begin{itemize}
          \item \textbf{Graph Topology:} Native property graph stores nodes and edges directly.
          \item \textbf{Sub-15ms Matching:} Hybrid ranking metrics calculated inside database memory.
        \end{itemize}
      \end{scard}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{scard}{Multi-Role Portals}
        \begin{itemize}
          ...
          \item \textbf{Investors:} Set ticket bounds, check match lists, transfer funds.
          \item \textbf{Analysts:} Verify milestones, top up wallets, inspect network paths.
        \end{itemize}
      \end{scard}
    \end{column}
  \end{columns}
  ...
  \begin{successbox}{Unified Value Proposition}
    Async FastAPI API + Neo4j Graph traversals + Redis caching \& mutex locks = Scaleable, secure startup directory.
  \end{successbox}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{tabularx}{\textwidth}{lXX}
        ...
        \midrule
        Traversals & Index-free adjacency & Recursive JOIN tables \\
        Edge Props & Native properties & Association entities \\
        Flexibility & Schemaless labels & Rigid DDL migrations \\
        Path query & Bolt protocol ($<$2ms) & Multiple query loops \\
        \bottomrule
      \end{tabularx}
      \vspace{4mm}
    ...
    \begin{column}{0.42\textwidth}
      \begin{scard}{Use Case Rationale}
        \begin{itemize}
          \item \textbf{Natural Graph Structure:} Startup ecosystem is inherently relational. Connecting startups, founders, investors, and milestones via direct physical links.
...
\begin{frame}{Why Redis? — High-Performance Caching}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{tabularx}{\textwidth}{lXX}
        \toprule
        \textbf{Feature} & \textbf{With Redis Cache} & \textbf{Without Redis} \\
        \midrule
        Feed queries & Cached hit ($<$2ms) & DB query scan (50ms) \\
        Match lists & TTL-based cache lookup & On-the-fly math loop \\
        Round locks & Atomic lease (SET NX) & DB lock contention \\
        Leaderboards & Redis ZSET sorted list & Aggregated group-by \\
        \bottomrule
      \end{tabularx}
      \vspace{4mm}
    ...
    \begin{column}{0.42\textwidth}
      \begin{scard}{Key Workflow States}
        \begin{itemize}
          ...
          \item \textbf{Match feed:} Retrieve matched investors based on the scoring parameters.
        \end{itemize}
      \end{scard}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{scard}{Code \& Build Automation}
        \begin{itemize}
          \item \textbf{Git / GitHub:} Repository hosting source code and deployment manifests.
          \item \textbf{Jenkins CI/CD:} 6-stage pipeline automation defined in \texttt{Jenkinsfile}.
          \item \textbf{GitHub Actions:} Automated PR workflow validating code syntax.
        \end{itemize}
      \end{scard}
      \vspace{2mm}
      \begin{scard}{Virtualization \& Runtime}
        \begin{itemize}
          ...
          \item \textbf{Docker Compose:} Configures isolated bridge networks and persistent storage volumes.
        \end{itemize}
      \end{scard}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{scard}{Serving \& Storage}
        \begin{itemize}
          \item \textbf{Nginx:} Web server and reverse proxy handling static/dynamic endpoints.
          \item \textbf{Neo4j \& Redis:} Property graph database and in-memory key-value cache.
          \item \textbf{Pytest Testing:} Boundary verification testing framework.
        \end{itemize}
      \end{scard}
    \end{column}
  \end{columns}
